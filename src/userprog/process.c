...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  while(child->alive)
  {
    ...
    child->waiting_parent = thread_current();
    child->already_called = true;
    thread_block();
...
bool
load (const char *file_name, void (**eip) (void), void **esp, char *tokens[]) 
{
  struct thread *t = thread_current ();
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      spage_insert(thread_current(), upage, kpage, writable); 
  ...
  if (kpage != NULL) 
    {
      ...
      if (success)
      {
        spage_insert(thread_current(), ((uint8_t *) PHYS_BASE) - PGSIZE, kpage, true);
        *esp = PHYS_BASE;
        ...
        {
          *esp -= (strlen(tokens[token_num-count-1]) + 1);
          strlcpy((char *)(* esp), tokens[token_num-count-1], strlen(tokens[token_num-count-1])+1);
          count++;
        }
        ...
        *(uintptr_t *)(* esp) = 0;
        ...
        {
          *esp -= 4;
          tok_addr -= (strlen(tokens[token_num-k-1])+1);
          *(uintptr_t *)(* esp) = tok_addr;
        }
